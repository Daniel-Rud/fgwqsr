...
generate_optim_limits = function(effects, cov_mat, zero_threshold_cutoff)
{
  ...
  groups_on_boundary = setdiff(groups_on_boundary,one_pollutant_groups)
  ...
  }else # if there are nuisance groups with entire group on boundary
  {
    ...
    permutations_regions = permutations(n = 2, r = groups_on_boundary %>% length, 
...
mvn_lrt_simulation= function(effects, cov_mat, group_num, pollutant_num = NA, 
                             num_sims = 10000, zero_threshold_cutoff = .5, 
                             cores = availableCores(), seed = 2023)
{
  ...
  # simulate the MVN 1 sample realizations 
  mvn_samples = rmvnorm(num_sims, mean = effects %>% unlist, sigma = cov_mat)
  inv_cov_mat = pinv(cov_mat) # used in optim, use moore pentrose psuedoinverse
  ...
  if((num_groups == 1) && (is.na(pollutant_num)))
  {
    ...
    ll_H0 = -1*future_apply(mvn_samples, MARGIN = 1, FUN = function(x) 
    {
      return(mvn_log_likelihood(mean = rep(0, num_in_each_group), sample = x, inv_cov_mat = inv_cov_mat))
    })
    
    ll_H1 = future_apply(mvn_samples, MARGIN = 1, 
  ...
  }else # if we are performing group LRT with more than one group OR single pollutant LRT
  {
    ...
    ll_H0 = future_apply(mvn_samples, MARGIN = 1, 
...
lrt_optimization_H0 = function(mvn_sample, inv_cov_mat,optimization_limits_list, affected_pollutants)
{
  ...
  for(i in 1: length(optimization_limits_list))
  {
    ...
    ll_vals[i] = tryCatch(expr = 
                            {
                              -1*optim(initial_values, fn = mvn_log_likelihood_H0, 
...
lrt_optimization_H1 = function(mvn_sample, inv_cov_mat,optimization_limits_list)
{
  ...
  for(i in 1: length(optimization_limits_list))
  {
    ...
    ll_vals[i] = tryCatch(expr = 
    {
      -1*optim(initial_values, fn = mvn_log_likelihood, 
                             ...
                             verbose)
{
  ...
  if(verbose == TRUE)
  {
    ...
    p <- progressor(steps = nrow(inference_order))
    handlers("progress")
  ...
  p_val_adj = (2*(1-pnorm(abs(Z_adj_params)))) 
  
  # compute CIs 
  
  ci_95_lower = adj_params - qnorm(.975)*sqrt(adj_params_var) 
  ci_95_upper = adj_params + qnorm(.975)*sqrt(adj_params_var)
