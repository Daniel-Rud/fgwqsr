...
# Introduction 
<br>
In this vignette, we will showcase how to fit FGWQSR models on cross sectional data with a binary outcome.  FGWQSR estimates parameters to the following model: 
\begin{align}
y_i &\sim \text{Bernoulli}(\pi_i) \\ 
\text{logit}(\pi_i) &= c_0+ \sum\limits_{g = 1}^G \gamma_g \bigg( \sum\limits_{k = 1} ^{c_g} w_{g,k} \cdot q_{g,k,i}\bigg) + \sum_{r = 1}^R \phi_rz_{r,i}  \label{eq:2}
\end{align}
where 
...
```{r packages}
source("R/fgwqsr_updated_source.R")
library(dplyr) # for %>% pipe operator
library(mvtnorm) # for mvtnorm()
library(statar) # for xtile()
...
We create a function to construct the desired correlation matrix.  
```{r}
...
create_corr_mat = function(ESS, CCS)
{
  ...
  corr_mat = make.positive.definite(corr_mat) # in case it is not positive definite 
  return(corr_mat)
}
...
chem_data = rmvnorm(n, mean = rep(0, 14), sigma = corr)
```
<br>
...
```{r}
chem_data = apply(chem_data, MARGIN = 2, xtile, n = 5) - 1
...
<br>
From the FGWQSR package, the main function we will be using to fit models is the fgwqsr() function.  The model formula that we specify will be different than traditional formulas in lm and glm, as we will need to denote our mixture groups.  Three special characters are used in fgwqsr formulas: 
<br>
...
We can see the model summary using the call fgwqsr_summary()

```{r}

fgwqsr_summary(fgwqsr_fit)
...
```
<br>
Again, we use the function fgwqsr_summary to view the results of the fgwqsr() call.  

```{r}

fgwqsr_summary(fgwqsr_fit_adj)
